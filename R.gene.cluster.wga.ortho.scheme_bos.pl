...
close IN;
my @accs2=("LGP01","LGP02","LGP03","LGP04","LGP05");
my @accs1 =("LGP01","LGP02","brahman","LGP03","LGP04","LGP05");
my @accs3=("brahman","LGP01","LGP02","LGP03","LGP04","LGP05");
my %reg;
...
foreach my $clu (sort keys %clu) {
   ...
   #print $clu,"\n";
   my $ortRegFile = "/home/sarwar/abhisek/gene_synteny_fig/orthofinder/OrthoFinder/Results_Jul10/Orthogroups/Orthogroups.txt";
   if (! -e $ortRegFile) {
...
sub grToCSV { ## change the output of orthoFinder into tab file including each accession infor
  ...
  while (<IN>) {
  	 ...
  	 foreach my $gene (@t) {
  	 	if ($gene=~/BI\d/) {
  	 	  push @{$gene{"brahman"}},$gene;	
  	 	}elsif ($gene=~/BILGP01/) {
  	 	  push @{$gene{"LGP01"}},$gene;	
  	 	}
  	 	elsif ($gene=~/BILGP02/) {
  	 	  push @{$gene{"LGP02"}},$gene;	
  	 	}
  	 	elsif ($gene=~/BILGP03/) {
  	 	  push @{$gene{"LGP03"}},$gene;	
  	 	}
  	 	elsif ($gene=~/BILGP04/) {
  	 	  push @{$gene{"LGP04"}},$gene;	
  	 	}
  	 	elsif ($gene=~/BILGP05/) {
  	 	  push @{$gene{"LGP05"}},$gene;	
  	 	}
  	 }
...
sub getAln {
	...
	foreach my $i (0..$#accs-1) {
   	  my $acc1 = $accs[$i];
   	  my $acc2 = $accs[$i+1];
   	  my $alnFile = "$alnDir/$acc1/$acc2.wga.block.txt";
   	  open IN,$alnFile;
...
sub drawFig2 {
   ...
   print "#figure size $outSVG $width  x  $height\n";     
    ...
    my $endRef = $reg{"brahman"}[2];
	my $startRef = $reg{"brahman"}[1];
	$svg->text(id=>"Col-0", x=>$x0-30, y=>$y0, "font-family"=>"Arial","font-size"=>$fsize*2, -cdata => "brahman", );
	...
	foreach my $gene (sort keys %{$genes{"brahman"}}) {
		my ($id,$start,$end,$direc) = @{$genes{"brahman"}{$gene}};
		my $color = $lblue;      
		#print "gene $gene\n";  	
		if ($gr{$gene}) {
		   my $grNo = $gr{$gene};
